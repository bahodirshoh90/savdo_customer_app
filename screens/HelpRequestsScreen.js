...
export default function HelpRequestsScreen() {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      setSnackbar({ visible: true, message: e.response?.data?.detail || e.message || 'So\'rovlarni yuklashda xatolik' });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleSend = async () => {
    ...
    try {
      await api.post('/help-requests', { message });
      closeDialog();
      fetchRequests();
      setSnackbar({ visible: true, message: 'So\'rov yuborildi' });
    } catch (e) {
      setSnackbar({ visible: true, message: e.response?.data?.detail || e.message || 'Yuborishda xatolik' });
    }
  };
