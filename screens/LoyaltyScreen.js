  ...
  const loadLoyaltyData = async () => {
    try {
      ...
      const [pointsData, transactionsData] = await Promise.all([
        api.get('/loyalty/points').catch((e) => {
        ...
        api.get('/loyalty/transactions?limit=50').catch((e) => {
          if (e?.response?.status === 401) return null;
          throw e;
        }),
      ]);

      if (pointsData) setLoyalty(pointsData);
      if (transactionsData) setTransactions(Array.isArray(transactionsData) ? transactionsData : []);
    } catch (error) {
