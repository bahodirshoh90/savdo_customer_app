...
export default function OfflineSyncScreen({ navigation }) {
  ...
  const loadOfflineOrders = async () => {
    try {
      ...
      showToast('Offline buyurtmalarni yuklashda xatolik', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSync = async () => {
    if (orders.length === 0) {
      showToast('Sinxronizatsiya qilish uchun buyurtmalar yo\'q', 'error');
      return;
    }
        ...
        { text: 'Bekor qilish', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              if (result.synced > 0) {
                showToast(`${result.synced} ta buyurtma sinxronizatsiya qilindi`, 'success');
                await loadOfflineOrders();
              } else if (result.error) {
                throw result.error;
              } else {
                showToast('Sinxronizatsiya qilishda xatolik', 'error');
              }
            } catch (error) {
