    ...
    try {
      ...
      if (page === 1) {
        try {
          ...
          console.warn('[ORDERS] Offline sync failed:', syncError?.message);
        }
      }
  ...
  useEffect(() => {
    ...
    const unsubscribeStatusUpdate = websocketService.on('order_status_update', (message) => {
      ...
      if (data && data.order_id) {
        ...
        // Update order status in-place to avoid full reload
        setOrders(prev =>
          prev.map(o =>
            o.id === data.order_id ? { ...o, status: newStatus } : o
          )
        );
        
        // If filter excludes new status, switch to 'all' (useFocusEffect will reload)
        if (newStatus === 'completed' && statusFilter !== 'all' && statusFilter !== '') {
          setStatusFilter('all');
        }
      ...
      <View style={[styles.filterContainer, { backgroundColor: colors.background }]}>
        <View style={[styles.filterCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <View style={styles.filterHeader}>
            <Text style={[styles.filterLabel, { color: colors.text }]}>Holatni tanlang</Text>
          </View>
          ...
          ListFooterComponent={
            hasMore && orders.length >= ordersPerPage ? (
              <View style={styles.loadMoreContainer}>
                ...
                <Text style={[styles.loadMoreText, { color: colors.textLight }]}>Yuklanmoqda...</Text>
              </View>
            ) : null
          }
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Text style={[styles.emptyText, { color: colors.textLight }]}>Buyurtmalar topilmadi</Text>
            </View>
          }
        />
      )}
